...
public class DashboardApplyLeaveTest extends BaseTest {
    ...
    public void dashboardApplyLeaveTest(TestData testData) {

        LoginPage loginPage = new LoginPage();
        HomePage homePage = loginPage.loginToApplication(testData.getUsername(), testData.getPassword());

        homePage.clickOnApplyLeave();

//        Assert.assertTrue(homePage.getPageURL().contains(testData.getExpectedUrl()),
//                "Assertion for Apply Leave URL");

        VerificationUtils.validateResponse(homePage.getPageURL().contains(testData.getExpectedUrl()+"_FAIL"),
                ASSERTION_FOR +" -  <b> <u> Apply Leave URL </b> </u>");
}
